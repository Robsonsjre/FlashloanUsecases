...
contract Arbitrageur is Addresses {
  ...
  constructor() public {
    ...
    exchangeA = UniswapExchangeInterface(uniswapFactoryA.getExchange(DAI_ADDRESS));

    uniswapFactoryB = UniswapFactoryInterface(UNISWAP_FACTORY_B);
    exchangeB = UniswapExchangeInterface(uniswapFactoryB.getExchange(DAI_ADDRESS));
  ...
  function executeOperation(
    ...
    uint256 daiBought = exchangeB.ethToTokenSwapInput.value(ethBought)(1, deadline);
  ...
  function imbalanceExchanges(uint256 _amount) external {
    ...
    exchangeB.tokenToEthTransferInput(_amount, 1, getDeadline(), msg.sender);
  }
}
